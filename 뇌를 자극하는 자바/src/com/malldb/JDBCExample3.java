...
class JDBCExample3 {
    public static void main(String args[]) {
    	...
    	try {
            ...
            rs = stmt.executeQuery("select scode,	num  from stockinfo");
            
            System.out.println("상품코드      재고수량");
            
            while(rs.next()) {
                
            	scode = rs.getString(1); //rs.getString("jumincd");
            	num   = rs.getString(2); //rs.getString("pname");
            
                System.out.println(scode + "-----------" + num);   
            }
